...
(function () {
  ...
  /* Vložené náhledy: v náhledu se nic nestahuje ze sítě, takže obrázky
     musí být přímo v datech. Používají je příspěvky i fotky produktů. */
  const THUMBS = [
  ...
  const answers = {
    ...
    /*
     * Fotky produktů. Náhledy jsou vložené jako data: adresy, aby se
     * v náhledu nic nestahovalo ze sítě; přípona proto chodí zvlášť —
     * v aplikaci ji počítá hlavní proces z adresy ve feedu.
     */
    'media:products': (function () {
      var foto = function (i) { return THUMBS[i % THUMBS.length]; };
      var produkt = function (code, title, id, pripony, at) {
        var images = pripony.map(function (ext, i) { return { url: foto(i), ext: ext }; });
        var webp = pripony.filter(function (e) { return e === 'webp'; }).length;
        return {
          code: code, productId: id, title: title,
          url: 'https://quentino.cz/' + code, thumb: foto(0), images: images, webp: webp,
          state: webp === 0 ? 'none' : (webp === pripony.length ? 'webp' : 'mixed'),
          convertedAt: at || ''
        };
      };
      return {
        items: [
          produkt('QK-118', 'Kožený pásek Quentino — hnědý', '1351', ['jpg', 'jpg', 'jpg', 'png']),
          produkt('QM-042', 'Manžetové knoflíčky Onyx', '1352', ['webp', 'jpg', 'jpg']),
          produkt('QK-221', 'Kravata Bordó se vzorem', '1353', ['jpg', 'jpg']),
          produkt('QMO-07', 'Motýlek Perla', '1354', ['jpg', 'png', 'jpg'],
            new Date(Date.now() - 600000).toISOString()),
          produkt('QKS-12', 'Kšandy tmavě modré', '1355', ['jpg', 'jpg', 'jpg'])
        ],
        total: 37, offset: 0, limit: 120
      };
    })(),
