...
import type { DigestDay, DigestReport, DigestSlice, DigestTask, DigestTotals, DigestTurn } from '@shared/types';
import { api } from '../api';
            ...
            <>
              ...
              <div className="dg-card dg-insight">
                <div className="dg-card-head">
                  ...
                  {report.insight && <span className="dg-when">{since(report.insight.at)}</span>}
                  <button className="dg-again" disabled={busy} onClick={() => load(true)}>
                    {busy ? 'Počítám…' : 'Přegenerovat'}
                  </button>
                </div>
                {!report.insight && !report.insightError && (
                ...
                {report.insight && (
                  <>
                    <p className="dg-headline">{report.insight.headline}</p>
                    ...
                    {report.insight.focus && (
                      <p className="dg-focus">Příště se chce podívat na: {report.insight.focus}</p>
                    )}
                  </>
                )}
              </div>
              ...
              <div className="dg-card">
                ...
                {turns.length === 0 && (
                  <div className="dg-chips">
                    {(report.insight?.questions?.length
                      ? report.insight.questions
